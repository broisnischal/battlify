...
- The daemon now gates discharge on **physical adapter presence** (AppleSmartBattery `ExternalConnected`), which stays true through a force-discharge, instead of the providing-source flag. Discharge now runs continuously until it reaches the limit (or the cable is genuinely unplugged).
